...
<div class="container">
    ...
    <?php
    $result =$connect->prepare('SELECT * FROM produit ORDER BY id_produit ASC');  //Request for the verification of login
    $result->execute();
    $result->rowCount();
    if( $result->rowCount() > 0)
    {
        while($row = $result->fetch())
        {
            ?>
            <div class="col-md-4">
                <form method="post" action="test.php?action=add&id=<?php echo $row["id_produit"]; ?>">
                    <div style="border:3px solid #5cb85c; background-color:whitesmoke; border-radius:5px; padding:16px;" align="center">
                        <img src="../img/<?php echo $row["photo"]; ?>" class="img-responsive" /><br />
